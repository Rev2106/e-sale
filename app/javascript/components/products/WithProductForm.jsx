    ...
    checkErrors = (state, fieldName) => {
        ...
        switch(fieldName) {
            ...
            case 'description':
                ...
                break
            case 'price':
                if (parseFloat(state.fields.price) <= 0.0 ||
                !state.fields.price.toString().match(/^\d{1,}(\.\d{0,2})?$/)) {
                    error.price = 'Price has to be a positive number'
                }
                break
            case 'quantity':
                if (parseInt(state.fields.quantity, 10) <= 0 ||
                !state.fields.quantity.toString().match(/^\d{1,}$/)) {
